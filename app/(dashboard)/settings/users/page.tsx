...
const ROLE_COLORS: Record<string, { bg: string; color: string }> = {
  owner:         { bg: 'rgba(0,229,160,0.12)',  color: 'var(--sophi-green)' },
  doctor:        { bg: 'rgba(77,154,255,0.12)', color: 'var(--sophi-blue)' },
  physio:        { bg: 'rgba(77,154,255,0.12)', color: 'var(--sophi-blue)' },
  masseur:       { bg: 'rgba(77,154,255,0.12)', color: 'var(--sophi-blue)' },
  coach:         { bg: 'rgba(255,211,0,0.12)',  color: 'var(--sophi-warn)' },
  fitness_coach: { bg: 'rgba(255,211,0,0.12)',  color: 'var(--sophi-warn)' },
  nutritionist:  { bg: 'rgba(180,141,252,0.12)', color: 'var(--sophi-purple)' },
  director:      { bg: 'rgba(180,141,252,0.12)', color: 'var(--sophi-purple)' },
  scout:         { bg: 'rgba(180,141,252,0.12)', color: 'var(--sophi-purple)' },
  team_manager:  { bg: 'rgba(180,141,252,0.12)', color: 'var(--sophi-purple)' },
  athlete:       { bg: 'rgba(255,255,255,0.08)', color: 'var(--sophi-text2)' },
}
  ...
  return (
    <div className="relative">
      <button
        ...
        style={{ background: 'var(--sophi-bg3)', color: 'var(--sophi-text2)' }}
      >
      ...
      {open && (
        <>
          ...
          <div
            ...
            style={{ background: 'var(--sophi-bg2)', borderColor: 'var(--sophi-border)', minWidth: 180 }}
          >
            {allSquads.length === 0 ? (
              <div className="px-3 py-2 text-xs" style={{ color: 'var(--sophi-text3)' }}>Sem squads na organização</div>
            ) : (
              allSquads.map((squad) => {
                ...
                return (
                  <button
                    ...
                    className="w-full flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-[var(--sophi-bg3)]"
                  ...
                  >
                    {saving === squad.id ? <Loader2 size={11} className="animate-spin shrink-0" /> : (
                      <span
                        ...
                        style={{ borderColor: 'var(--sophi-border2)', background: assigned ? 'var(--sophi-green)' : 'transparent' }}
                      >
    ...
    <div className="relative">
      ...
      {open && (
        <>
          ...
          <div
            className="absolute right-0 top-full mt-1 z-20 rounded-lg border shadow-xl overflow-hidden"
            style={{ background: 'var(--sophi-bg2)', borderColor: 'var(--sophi-border)', minWidth: 160 }}
          >
            {ROLE_OPTIONS.map((r) => {
              ...
              return (
                <button
                  ...
                  className="w-full flex items-center gap-2 px-3 py-2 text-left text-xs hover:bg-[var(--sophi-bg3)] disabled:opacity-40 disabled:cursor-not-allowed"
                  style={{ color: 'var(--sophi-text)' }}
                >
                  <span className="w-3 h-3 rounded-full shrink-0" style={{ background: s.color }} />
                  {ROLE_LABELS[r] ?? r}
                  {r === currentRole && <Check size={11} className="ml-auto" style={{ color: 'var(--sophi-green)' }} />}
                </button>
              )
            })}
          </div>
    ...
    <div className="max-w-2xl space-y-6">
      ...
      >
        <div className="flex items-center gap-2">
          <UserPlus size={14} style={{ color: 'var(--sophi-green)' }} />
          <span className="text-sm font-medium" style={{ color: 'var(--sophi-text)' }}>{t('inviteUser')}</span>
        </div>
        <div className="grid grid-cols-2 gap-3">
          <div className="space-y-1.5">
            <label className="text-xs" style={{ color: 'var(--sophi-text2)' }}>{t('inviteEmail')}</label>
            <input
              value={inviteEmail}
          ...
          <div className="space-y-1.5">
            <label className="text-xs" style={{ color: 'var(--sophi-text2)' }}>{t('inviteRole')}</label>
            <select
              value={inviteRole}
        ...
        <div
          ...
          style={{ background: 'rgba(255,211,0,0.08)', borderColor: 'rgba(255,211,0,0.2)', border: '1px solid', color: 'var(--sophi-warn)' }}
        >
          {t('inviteComingSoon')}
        </div>
      </div>
      ...
      <div className="space-y-2">
        {users.length === 0 ? (
          ...
          >
            ...
            <p className="text-sm" style={{ color: 'var(--sophi-text3)' }}>{t('noUsers')}</p>
          </div>
        ) : (
          users.map((u) => {
              ...
              >
                <div className="flex items-center gap-3">
                  <div
                    ...
                    style={{ background: 'rgba(0,229,160,0.1)', color: 'var(--sophi-green)' }}
                  >
