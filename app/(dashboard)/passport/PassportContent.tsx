...
export function PassportContent({ initialAthletes }: Props) {
  const [athletes] = useState<PassportAthleteDTO[]>(initialAthletes)
  const [selected, setSelected] = useState<string>(initialAthletes[0]?.id ?? '')
  ...
  const injuries = (athlete?.injury_events ?? []).sort((a, b) =>
        ...
        <div className="g2">
            ...
            <div className="card" style={{ marginBottom: 12, borderColor: isShareable ? 'rgba(0,229,160,0.3)' : 'var(--border)' }}>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 14 }}>
                {[
                  ...
                  { label: 'HRV BASELINE', value: (passport?.passport_data as { hrv_baseline_ms?: number } | undefined | null)?.hrv_baseline_ms ? `${(passport?.passport_data as { hrv_baseline_ms?: number })?.hrv_baseline_ms} ms` : '—', color: 'var(--purple)' },
            ...
            {profile && (
              <div className="card">
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                  {[
                    { label: 'CARGA MÉDIA', val: profile.avg_daily_load, desc: profile.avg_daily_load != null ? `${profile.avg_daily_load} UA` : '—' },
                    { label: 'CARGA PICO', val: profile.peak_load, desc: profile.peak_load != null ? `${profile.peak_load} UA` : '—' },
                  ].map(p => (
                    <div key={p.label} style={{ background: 'var(--bg3)', borderRadius: 8, padding: 10 }}>
                      ...
                      <div style={{ fontSize: 14, fontWeight: 600, color: 'var(--text)' }}>{p.desc}</div>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div>
            <div className="card">
                ...
                {isShared ? (
                  <div style={{ background: 'var(--green4)', border: '1px solid rgba(0,229,160,0.2)', borderRadius: 8, padding: 12 }}>
                    ...
                    <div style={{ fontSize: 12, color: 'var(--text2)', lineHeight: 1.8 }}>
                      ...
                      {(passport?.passport_data as { hrv_baseline_ms?: number } | undefined | null)?.hrv_baseline_ms && <>✓ HRV baseline: {(passport?.passport_data as { hrv_baseline_ms?: number })?.hrv_baseline_ms} ms<br /></>}
              ...
              {/* Passport notes */}
              {(passport?.passport_data as { notes?: string } | undefined | null)?.notes && (
                <div className="alert alert-w">
                  <div className="adot ad-w" />
                  <div style={{ fontSize: 12 }}>{(passport?.passport_data as { notes?: string })?.notes}</div>
                </div>
              )}
            </div>
          </div>
