      ...
      >
        <nav className="flex-1 space-y-6 px-3 py-4">
          {SECTIONS.map((section) => (
            <div key={section}>
              <p className="mb-1 px-3 text-[10px] font-semibold uppercase tracking-widest" style={{ color: 'var(--aura-text3)', fontFamily: 'var(--font-dm-mono)' }}>
                {section}
              </p>
                ...
                {NAV_ITEMS.filter((item) => item.section === section).map((item) => {
                  ...
                  return (
                    <li key={item.href}>
                      ...
                      >
                        <Icon size={15} className={active ? 'text-[var(--aura-green)]' : 'text-[var(--aura-text3)]'} />
        ...
        <div className="flex items-center gap-2 px-6 py-3 text-[10px]" style={{ color: 'var(--aura-text3)', fontFamily: 'var(--font-dm-mono)' }}>
          <LockKeyhole size={12} />
          INTERNAL ADMIN
        </div>
      </aside>
