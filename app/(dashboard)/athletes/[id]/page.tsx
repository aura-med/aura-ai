...
async function AthleteDetailContent({
  ...
  // Fetch athlete + current user profile in parallel
  const [{ data: athlete, error }, { data: { user } }] = await Promise.all([
    supabase
      .from('athletes')
      .select(`
  ...
  // Fetch viewer role + latest recommendations in parallel
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="space-y-4">
                      ...
                      <p className="text-xs leading-snug" style={{ color: 'var(--aura-text)' }}>
                        ...
                        {inj.is_recurrence && (
                          <span
                            ...
                            style={{ background: 'rgba(255,77,109,0.15)', color: 'var(--aura-danger)' }}
                          >
                            Recidiva
                          </span>
                        )}
          ...
          {activeRecData && riskLevel ? (
            ...
            // Score exists but no log yet (first load before score API was called)
            <div
              className="rounded-xl border p-5"
              style={{ background: 'var(--aura-bg2)', borderColor: 'var(--aura-border)' }}
            >
              <h3 className="text-sm font-semibold mb-2" style={{ color: 'var(--aura-text)', fontFamily: 'var(--font-syne)' }}>
                Recomendações
              </h3>
